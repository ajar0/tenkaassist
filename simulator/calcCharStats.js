const calcCharPotential = (from, to, potentialType, rarity) => {
  const result = {
    items: {},
    money: 0,
  ...
  let type = rarity < 2 ? 3 : potentialType ?? 1;
...
function calcCharStats ({
  ...
  const buff = validPotentialSub.reduce((res, boolean, i) => {
    ...
    return res;
  }, calcCharPotential([1, 0], [potential - 1, 6], potentialType).buff);
